...
''' SIMULATE NETWORK '''
def simulateNetwork(mode,Nplls,F,F_Omeg,K,Fc,delay,feedback_delay,dt,c,Nsteps,topology,couplingfct,histtype,phiS,phiM,domega,diffconstK,diffconstSendDelay,cPD,Nx=0,Ny=0,Trelax=0):
	...
	pll_list = generatePllObjects(mode,topology,couplingfct,histtype,Nplls,dt,c,delay,feedback_delay,F,F_Omeg,K,Fc,y0,phiM,domega,diffconstK,diffconstSendDelay,Nx,Ny,cPD,Trelax)	# create object lists of PLL objects of the network
	...
	return {'phases': phi, 'intrinfreq': omega_0, 'coupling_strength': K_0, 'transdelays': delays_0, 'cPD': cPD_t}

''' CREATE PLL LIST '''
def generatePllObjects(mode,topology,couplingfct,histtype,Nplls,dt,c,delay,feedback_delay,F,F_Omeg,K,Fc,y0,phiM,domega,diffconstK,diffconstSendDelay,Nx,Ny,cPD,Trelax=0):
	if not Nplls == Nx*Ny:
	...
	elif cPD>0 and Trelax>0 and c==0:
		if couplingfct == 'sin':
			# print('Initiate (phase shifted) PLL objects. Simulate with additive noise, sinusoidal coupling function.')
			pll_list = [ PhaseLockedLoop(										# setup PLLs and storage in a list as PLL class objects
								Delayer(delay,dt,feedback_delay,diffconstSendDelay),			# delayer takes a time series and returns values at t and t-tau
