...
''' SIMULATION CALL '''
def simulatePllNetwork(mode,topology,couplingfct,F,Nsteps,dt,c,Fc,F_Omeg,K,N,k,delay,phiS,phiM,domega,cLF,diffconstK,diffconstSendDelay,Nx=0,Ny=0,kx=0,ky=0,isPlottingTimeSeries=False):
	''' SIMULATION OF NETWORK '''
	simresult = sim.simulateNetwork(mode,N,F,F_Omeg,K,Fc,delay,dt,c,Nsteps,topology,couplingfct,phiS,phiM,domega,diffconstK,diffconstSendDelay,cLF,Nx,Ny) # kx and ky do not need to be handed over - already in phiM contained
...
if __name__ == '__main__':
	...
	if not unit_cell.is_inside((phiS), isRotated=False):						# check, whether point phiSr belongs to the unit cell
		...
		data = simulatePllNetwork(mode,topology,couplingfct,F,Nsteps,dt,c,Fc,F_Omeg,K,N,k,delay,phiS,phiM,domega,cLF,diffconstK,diffconstSendDelay,Nx,Ny,kx,ky,False)
